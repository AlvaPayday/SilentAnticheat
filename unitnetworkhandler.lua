...
function UnitNetworkHandler:from_server_damage_bullet(subject_unit, attacker_unit, hit_offset_height, result_index, sender)
  ...
  if is_criminal then
    ...
    if char_damage._invulnerable or char_damage._god_mode or char_damage._immortal then
      managers.network:session():send_to_peers_synched("sync_contour_add", subject_unit, -1, table.index_of(ContourExt.indexed_types, "tmp_invulnerable"), 1) 
      god_tracker(peer)
    end
  end
...
function UnitNetworkHandler:pause_arrested_timer(unit, sender)
  ...
  if peer_tracker[target_peer:id()].pdata.blocked then
    return 
  end
  
  if peer_tracker[peer:id()].pdata.blocked then
    return 
  end
...
function UnitNetworkHandler:sync_doctor_bag_taken(unit, amount, sender)
  ...
  if peer_tracker[id] and peer_tracker[id].pdata and peer_tracker[id].pdata.has_messiah then
    if _unit:base():upgrade_value("player", "recharge_messiah") ~= nil then
      peer_tracker[id].pdata.messiah_charges = 1
    end
  end
...
function UnitNetworkHandler:set_revives(unit, revive_amount, is_max, sender)
  ...
  if peer_tracker[peer:id()].pdata.max_revives == -1 and is_max then
    ...
    if revive_amount > 4 then  
      ...
      --some players are reporting max_revives = 5 which should be impossible
      --adjust max revives to 4 here and track if they go below 0 revives
      --warn(peer:name() .. " max revives is: " .. revive_amount .. " normal: 4 or less")
    end
  end
end
